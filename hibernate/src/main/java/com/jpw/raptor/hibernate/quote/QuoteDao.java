...
public interface QuoteDao extends CrudRepository<Quote, QuoteId> {
    ...
    */
    @Query(
            ...
            nativeQuery = true)
    public void delete(String symbol, String date);
    ...
    */
    @Query(
            value = "SELECT * FROM quote_tbl WHERE symbol=?1 and date_tx>? and date_tx<?2 ORDER BY date_tx ASC",
            nativeQuery = true)
    public List<Quote> getByYear(String symbol, String startDate, String endDate);
    ...
    @Query(
            value = "SELECT * FROM quote_tbl WHERE symbol=?1 and date_tx>=?2 ORDER BY date_tx ASC",
            nativeQuery = true)
    public List<Quote> getForPerformance(String symbol, String date);
